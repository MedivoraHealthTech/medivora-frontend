...
import { FlaskConical, ClipboardList, LogOut, Menu, X } from 'lucide-react'

const NAV = [
  { path: '/lab', label: 'Test Orders', icon: ClipboardList },
]
...
export default function LabLayout() {
  ...
  useEffect(() => {
    if (!localStorage.getItem('medivora_lab_token')) {
      navigate('/lab/login', { replace: true })
    }
  }, [navigate])

  function logout() {
    localStorage.removeItem('medivora_lab_token')
    localStorage.removeItem('medivora_lab_user')
    navigate('/lab/login', { replace: true })
  }

  const user = (() => { try { return JSON.parse(localStorage.getItem('medivora_lab_user') || '{}') } catch { return {} } })()
    ...
    <>
      <div style={{ padding: '1.25rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg,#1D4ED8,#0EA5E9)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <FlaskConical size={18} color="#fff" />
          </div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: 0.3 }}>Diagnostics</div>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', fontWeight: 600, letterSpacing: 1 }}>MEDIVORA LAB</div>
          </div>
        </div>
        {isMobile && (
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Inter', system-ui, sans-serif", background: '#f0f7ff' }}>
      {!isMobile && (
      ...
      <main style={{ marginLeft: isMobile ? 0 : 220, flex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        {isMobile && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: BG, position: 'sticky', top: 0, zIndex: 100 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 800, color: '#fff' }}>Lab Portal</span>
          </div>
        )}
