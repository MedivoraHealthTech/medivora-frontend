...
export default function PharmacyLoginPage() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (data.portal_type !== 'pharmacy') throw new Error('Invalid portal credentials')
      localStorage.setItem('medivora_pharmacy_token', data.token)
      localStorage.setItem('medivora_pharmacy_user', JSON.stringify(data))
  ...
  return (
    <div style={{
      ...
      justifyContent: 'center', background: 'linear-gradient(135deg, #f0fff4 0%, #e6fffa 100%)',
      ...
      <div style={{
        ...
        boxShadow: '0 8px 40px rgba(0,128,96,0.1)',
        border: '1px solid #c6f6d5',
      }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{
            width: 56, height: 56, borderRadius: 16,
            background: 'linear-gradient(135deg, #00875A, #00C896)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            margin: '0 auto 1rem',
            boxShadow: '0 4px 16px rgba(0,135,90,0.3)',
          }}>
            <Pill size={26} color="#fff" />
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1a3a2a', marginBottom: 4 }}>
            Pharmacy Portal
          </h1>
          <p style={{ fontSize: 13, color: '#718096' }}>Medivora — Pharmacy Dashboard</p>
        </div>
        ...
        <form onSubmit={handleLogin}>
          <div style={{ marginBottom: 14 }}>
            ...
            <input
              ...
              placeholder="medivora_pharmacy" required style={inp}
          ...
          <button type="submit" disabled={loading} style={{
            ...
            background: loading ? '#a0aec0' : 'linear-gradient(135deg, #00875A, #00C896)',
            color: '#fff', cursor: loading ? 'not-allowed' : 'pointer',
            boxShadow: loading ? 'none' : '0 4px 16px rgba(0,135,90,0.3)',
            transition: 'all 0.2s',
          }}>
